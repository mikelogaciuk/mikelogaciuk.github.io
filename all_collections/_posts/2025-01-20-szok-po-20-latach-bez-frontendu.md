...
- [Javascript i Typescript](#javascript-i-typescript)
  ...
  - [Backend](#backend)
    ...
    - [Koa.js](#koajs)
    - [NestJS](#nestjs)
    - [Pozostałe backend'owe zabawki z ekosystemu JS](#pozostałe-backendowe-zabawki-z-ekosystemu-js)
  - [Fullstack](#fullstack)
    - [Inertia](#inertia)
    - [Meteor](#meteor)
    - [NextJS](#nextjs)
...
_Natomiast nie wgryzałem się w to nadmiernie - gdyż mój obecny zadań ograniczał się do backend'u skupionego na danych ERP'owych i szerokorozumianym `bigdata` oraz `observability`, a tak naprawdę ostatni raz pisałem cokolwiek na froncie gdy byłem jeszcze w technikum czyli w okolicy 2005/2006 roku, gdzie prym na froncie wiódł PHP i JS w przeglądarce._
...
#### Pozostałe backend'owe zabawki z ekosystemu JS

Z ciekawych rzeczy, na które natrafiłem w trakcie researchu, to choćby `Manifest`, który jest ultralekkim jednoplikowym... **Uwaga!**: _backend'em_.
...
Jeżeli przyjdzie mi pisać coś lekkiego, co wymagać będzie jedynie połączenia z zewnętrznymi bazami, a dla siebie będzie jedynie wymagać kontroli użytkowników i poziomu uprawnień - to wybór raczej będzie prosty.
...
Tu chyba nie mam ochoty się nadmiernie rozpisywać, gdyż jestem zdania, że używanie `Rust`'a w web developmencie to jak bombardowanie epicentrum pandemii Eboli z pomocą broni jądrowej.
...
Ha! Tego nie będę opisywać, bo używam nagminnie każdego dnia, gdzie mam uruchomionych wiele mikroserwisów w trybie `HPA`, które zależnie od obciążenia są skalowane, a dodatkowo stoją za `LB` co znacznie ułatwia mi pracę.

Tzw. `rolling-releases` to także przydatna funkcja, która sprawia, że aplikacja jest dla klienta cały czas dostępna.

### Coolify

Ciekawym narzędziem i bardzo przydatnym jest ta open-source'owa alternatywa dla: `Heroku` / `Netlify` / `Vercel`.

Jest to samodzielnie hostowane środowisko uruchomieniowe, które stanowi alternatywę dla wyżej wymienionych płatnych rozwiązań.

Umożliwia ono zarządzanie serwerami, aplikacjami i bazami danych na własnym sprzęcie, wymagając jedynie połączenia SSH i obsługuje szeroki zakres języków programowania i frameworków, co pozwala na uruchamianie statycznych stron internetowych, API, backendów, baz danych i innych typów aplikacji.

Z jego pomocą można uruchamiać aplikacje na dowolnym serwerze, w tym na własnych serwerach: `VPS`, `Raspberry Pi`, `EC2`, `DigitalOcean` itd.

Umożliwia również integrację z platformami `GitHub`, `GitLab` czy `Bitbucket`. Obsługuje także automatyczne certyfikaty SSL, kopie zapasowe baz danych, webhooki, potężne API, terminal oraz monitorowanie i powiadomienia z alertami.
